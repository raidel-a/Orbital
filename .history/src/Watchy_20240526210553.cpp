  ...
  if (wakeupBit & MENU_BTN_MASK)
  {
    ...
    { // if already in menu, then select menu item
      ...
      case 0:
        ...
        break;
      case 1:
        showBuzz(settings.vibrateOClock, false);
        break;
      case 2:
        showAccelerometer();
        break;
...
void Watchy::showMenu(byte menuIndex, bool partialRefresh)
{
  ...
  const char *menuItems[] = {
      "About Watchy", "Buzz Interval", "Show Accelerometer",
...
void Watchy::showBuzz(byte menuIndex, bool partialRefresh)
{
  display.setFullWindow();
  display.fillScreen(GxEPD_BLACK);
  display.setFont(&FreeMonoBold9pt7b);
  ...
  while (1)
  {
    if (digitalRead(MENU_BTN_PIN) == 1)
    {
      // set settings.vibrateOClock to the index of the menu item
      settings.vibrateOClock = menuIndex;
      
    }
  
    if (digitalRead(DOWN_BTN_PIN) == 1)
    {
      // move down the menu, if at the end, go to the top, else increment
      menuIndex++;
      if (menuIndex > MENU_LENGTH - 1)
      {
        menuIndex = 0;
    ...
    {
      // move up the menu, if at the top, go to the bottom, else decrement
      menuIndex--;
      if (menuIndex < 0)
      {
        menuIndex = MENU_LENGTH - 1;
