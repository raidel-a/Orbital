...
void Watchy::showBuzz()
{
  ...
  uint8_t buzzInterval = settings.vibrateOClock;
  byte buzzIndex = 0;

  for (int i = 0; i < MENU_LENGTH; i++)
  {
    yPos = MENU_HEIGHT + (MENU_HEIGHT * i) + 20;
    display.setCursor(0, yPos);
    if (i == buzzIndex)
    {
      display.getTextBounds(buzzIntervals[i], 0, yPos, &x1, &y1, &w, &h);
      display.fillRect(x1 - 1, y1 - 10, 200, h + 15, GxEPD_WHITE);
      display.setTextColor(GxEPD_BLACK);
