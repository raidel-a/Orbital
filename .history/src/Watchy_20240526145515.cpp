...
void Watchy::init(String datetime)
{
  ...
  case ESP_SLEEP_WAKEUP_EXT0: // RTC Alarm
    ...
    case WATCHFACE_STATE:
      ...
      // if settings.vibrateOClock is 0, it will not vibrate
      // if (settings.vibrateOClock) is 
...
void Watchy::showBuzz()
{
  ...
  display.println("Choose Buzz Interval");
  // none, 1 min, 5 min, 15 min, 30 min, 1 hour
  const char *buzzIntervals[] = {"None", "1 min", "5 min", "15 min", "30 min", "1 hour"};
  int buzzInterval = settings.vibrateInterval;  
  int buzzIndex = 0;
  for (int i = 0; i < 6; i++)
  {
    display.setCursor(70, 100 + (20 * i));
    if (i == buzzInterval)
    {
      display.setTextColor(GxEPD_BLACK);
      display.println(buzzIntervals[i]);
      buzzIndex = i;
    }
    else
    {
  ...
  while (1)
  {
    if (digitalRead(MENU_BTN_PIN) == 1)
    {
      settings.vibrateInterval = buzzIndex;
      break;
    }
