...
void WatchyOrbital::drawWatchMinute()
{
    ...
    // Check if the current minute is near 0, 15, 30, or 45
    bool isSpecialTick = (currentTime.Minute <= 2 || (currentTime.Minute >= 13 && currentTime.Minute <= 17) ||
                          (currentTime.Minute >= 28 && currentTime.Minute <= 32) || (currentTime.Minute >= 43 && currentTime.Minute <= 47));

    int tickWidth = isSpecialTick ? 25 : 13; // Increase the width for special ticks

    fillArc2(tick, minuteAngle - tick, 88, tickWidth, primaryColor, tick);
    fillArc2(minuteAngle + (details ? 2 * tick : tick), 360 - tick, 85, 7, secondaryColor, tick);
